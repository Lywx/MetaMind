namespace MetaMind.Session.Operations
{
    ...
    public class OperationOrganizer
    {
        ...
        private void Organize(
            ...
            IList<IOperationDescription> groupedoperations,
            HashSet<string> groupedNames = null,
            int level = 1)
        {
            ...
            if (groupedNames != null
                && groupNames.SequenceEqual(groupedNames))
            {
                return;
            }
            ...
            var linkedoperations =
                new LinkedList<IOperationDescription>(groupedoperations);
            ...
            foreach (var group in groups)
            {
                ...
                if (!found)
                {
                    ...
                    var vacant = new OperationDescription(
                        @group.Key,
                        "",
                        @group.Value.Count != 0 ? @group.Value[0].Path : "");
            ...
            groupedoperations.RemoveRange(
                groupedoperations.Where(item => !groupNames.Contains(item.Name)));
            groupedoperations.AddRange(
                groupedoperations.Where(
                    item =>
                    groupNames.Contains(item.Name)
                    && !groupedoperations.Contains(item)));

            // Sorting in not essential for this process
            groupedoperations.Sort(
                (operation, other) => operation.CompareTo(other));
            ...
            if (level == 1)
            {
                operations.RemoveRange(
                    groupedoperations.Where(
                        item => !groupNames.Contains(item.Name)));
                operations.AddRange(
                    groupedoperations.Where(
                        item =>
                        groupNames.Contains(item.Name)
                        && !operations.Contains(item)));
        ...
        private HashSet<string> GroupNamesUnique(
            IList<IOperationDescription> groupedoperations,
            int level)
        {
            return
                groupedoperations.Select(
                    child => this.CropStart(child.Name, level))
                                 .ToHashSet();
        }
