    ...
    public class ViewItemBasicGraphics : ViewItemComponent, IItemGraphics
    {
        ...
        public virtual void Draw(GameTime gameTime, byte alpha)
        {
            ...
            if (this.Item.IsEnabled(ItemState.Item_Selected))
            {
                ...
                Primitives2D.FillRectangle(ScreenManager.SpriteBatch, this.ItemControl.NameFrame.Rectangle, this.ItemSettings.NameFrameHighlightColor);
            }
            else if (this.Item.IsEnabled(ItemState.Item_Editing))
            {
                Primitives2D.FillRectangle(ScreenManager.SpriteBatch, this.ItemControl.RootFrame.Rectangle, this.ItemSettings.RootFrameColor);
                Primitives2D.FillRectangle(ScreenManager.SpriteBatch, this.ItemControl.NameFrame.Rectangle, this.ItemSettings.NameFrameModificationColor);
            }
            else
            {
                Primitives2D.FillRectangle(ScreenManager.SpriteBatch, this.ItemControl.RootFrame.Rectangle, this.ItemSettings.RootFrameColor);
