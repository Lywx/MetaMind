namespace MetaMind.Engine.Component.Content.Asset
{
    ...
    public class AssetManager : GameControllableComponent, IAssetManager
    {
        ...
        private AssetXmlDocument document;

        public AssetList<FontAsset> Fonts { get; } = new AssetList<FontAsset>();

        public AssetList<ImageAsset> Images { get; } = new AssetList<ImageAsset>();
        ...
        public override void Initialize()
        {
            ...
            {
                var asset = this.AssetPath("Fonts", this.Fonts[i].Asset);
                asset = this.Interop.Content.UseArchive ? asset : Path.GetFullPath(asset);

                (this.Fonts[i].Resource) = this.Interop.Content.Load<SpriteFont>(asset);
            }

            for (int i = 0; i < this.Images.Count; i++)
            {
                this.Interop.Content.UseArchive = this.Images[i].Archive;
                string asset = this.AssetPath("Images", this.Images[i].Asset);
                this.Images[i].Resource = this.Interop.Content.Load<Texture2D>(asset);
            }
        ...
        {
            this.document = this.Content.Load<AssetXmlDocument>("Preload.xml");

            try
            {
                var e = this.document["Content"];
                if (e != null)
                {
                    this.LoadFonts(e);
                    this.LoadImages(e);
            ...
            {
                throw new Exception("Unable to load content file. " + e.Message);
            }

            base.LoadContent();
        }

        private void LoadFonts(XmlElement contentElement)
        {
            var fontElement = contentElement["Fonts"];
            if (fontElement == null)
            {
                return;
            }

            var fontList = fontElement.GetElementsByTagName("Font");
            if (fontList.Count > 0)
            {
                foreach (XmlElement font in fontList)
                {
                    var fontName = this.ReadAttribute(font, "Name", null, true);
                    var fontSize = int.Parse(this.ReadAttribute(font, "Size", null, true));
                    var fontPath = this.ReadAttribute(font, "Path", null, true);

                    var fontData = new FontAsset(fontName, fontSize, this.Content.Load<SpriteFont>(fontPath));
                    this.Fonts.Add(fontData);
                }
            }
        }

        private void LoadImages(XmlElement contentElement)
        {
            var imageElement = contentElement["Images"];
            if (imageElement == null)
            {
                return;
            }

            var imageList = imageElement.GetElementsByTagName("Image");
            if (imageList.Count > 0)
            {
                foreach (var image in imageList)
                {
                    var si = new SkinImage();
                    si.Name = this.ReadAttribute(image, "Name", null, true);
                    si.Archive = archive;
                    si.Asset = this.ReadAttribute(image, "Asset", null, true);
                    si.Addon = addon;
                    this.Images.Add(si);
                }
            }
        ...
        {
            if (this.document["Skin"]["Attributes"] == null) return;

            XmlNodeList l = this.document["Skin"]["Attributes"].GetElementsByTagName("Attribute");
        ...
        {
            if (this.document["Skin"]["Controls"] == null) return;

            XmlNodeList l = this.document["Skin"]["Controls"].GetElementsByTagName("Control");
