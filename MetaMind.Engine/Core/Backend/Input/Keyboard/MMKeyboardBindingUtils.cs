namespace MetaMind.Engine.Core.Backend.Input.Keyboard
{
    ...
    public static class MMKeyboardBindingUtils
    {
        ...
        public static MMKeyboardBinding<MMInputAction> Load<TActions>(IMMPlainConfigurationFileLoader loader)
            where TActions : MMInputActions
        {
            var actionBinding = new MMKeyboardBinding<MMInputAction>();

            foreach (var actionPair in MMPlainConfigurationLoader.LoadDuplicable(loader))
            {
