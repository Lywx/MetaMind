namespace MetaMind.Engine.Core.Backend.Content.Asset
{
    ...
    public class MMAssetManager : MMGeneralComponent, IMMAssetManager
    {
        #region Logger
        ...
        public void ReadPackage(string packageName)
        {
            ...
            try
            {
                ...
                if (e != null)
                {
                    var package = new MMPackageAsset(packageName);
        ...
        private void ReadFonts(MMPackageAsset package, XmlElement packageElement)
        {
            ...
            if (fontList?.Count > 0)
            {
                ...
                {
                    var fontName = MMXmlAttributeReader.ReadAttributeString(fontElement, "Name", null, true);
                    var fontSize = MMXmlAttributeReader.ReadAttributeInt(fontElement, "Size", 0, true);
                    var fontAsset = MMXmlAttributeReader.ReadAttributeString(fontElement, "Asset", null, true);
        ...
        private void ReadImages(MMPackageAsset package, XmlElement packageElement)
        {
            ...
            if (imageList?.Count > 0)
            {
                ...
                {
                    var imageName = MMXmlAttributeReader.ReadAttributeString(imageElement, "Name", null, true);
                    var imageAsset = MMXmlAttributeReader.ReadAttributeString(imageElement, "Asset", null, true);
        ...
        private MMImageDesign ReadImageDesign(XmlElement imageElement)
        {
            ...
            var screenWidth = MMXmlAttributeReader.ReadAttributeInt(designElement, "ScreenWidth", 0, true);
            var screenHeight = MMXmlAttributeReader.ReadAttributeInt(designElement, "ScreenHeight", 0, true);
