namespace MetaMind.Engine.Core.Services.IO
{
    ...
    public static class MMXmlAttributeReader
    {
        ...
        {
            var valueString = MMXmlAttributeFormatter.Color.ToArgbHexString(value);

            ReadAttributeString(valueElement, valueAttribute, ref valueString, MMXmlAttributeFormatter.Color.ToArgbHexString(valueDefault), valueRequired, valueInherited);

            try
            {
                value = MMXmlAttributeFormatter.Color.ParseArgbHex(valueString);
            }
            catch (Exception)
            {
                value = valueDefault;
        ...
        {
            var valueString = ReadAttributeString(valueElement, valueAttribute, MMXmlAttributeFormatter.Color.ToArgbHexString(valueDefault), valueRequired);

            try
            {
                return MMXmlAttributeFormatter.Color.ParseArgbHex(valueString);
            }
            catch (Exception)
            {
                return valueDefault;
