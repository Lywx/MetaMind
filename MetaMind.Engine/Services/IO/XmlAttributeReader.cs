namespace MetaMind.Engine.Services.IO
{
    ...
    public static class XmlAttributeReader
    {
        ...
        {
            var valueString = XmlAttributeFormatter.Color.ToArgbHexString(value);

            ReadAttributeString(valueElement, valueAttribute, ref valueString, XmlAttributeFormatter.Color.ToArgbHexString(valueDefault), valueRequired, valueInherited);

            try
            {
                value = XmlAttributeFormatter.Color.ParseArgbHex(valueString);
            }
            catch (Exception)
            {
                value = valueDefault;
        ...
        {
            var valueString = ReadAttributeString(valueElement, valueAttribute, XmlAttributeFormatter.Color.ToArgbHexString(valueDefault), valueRequired);

            try
            {
                return XmlAttributeFormatter.Color.ParseArgbHex(valueString);
            }
            catch (Exception)
            {
                return valueDefault;
        ...
        private static void ReadAttribute(XmlElement valueElement, string valueAttribute, ref object value, Type valueType, object valueDefault, bool valueRequired, bool valueInherited)
        {
            ...
            value = ValueReader.ReadValue(valueString, valueType, valueDefault);
        }
        ...
        private static object ReadAttribute(XmlElement valueElement, string valueAttribute, Type valueType, object valueDefault, bool valueRequired)
        {
            ...
            return ValueReader.ReadValue(valueString, valueType, valueDefault);
        }
    }
}