namespace MetaMind.Engine.Gui.Reactors
{
    ...
    public abstract class RenderReactor : MMReactor, IRenderReactor
    {
        ...
        public RenderReactorCollection Children { get; protected set; } = new RenderReactorCollection();

        public RenderReactor Parent { get; protected set; } = null;

        public RenderReactor Root { get; protected set; } = null;
        ...
        public virtual void Add(RenderReactor reactor)
        {
            if (reactor != null)
            {
                if (!this.Children.Contains(reactor))
                {
                    // Restore parent relationship before adding
                    reactor.Parent?.Remove(reactor);

                    // Configure parenthood
                    reactor.Enabled = (this.Enabled ? reactor.Enabled : this.Enabled);
                    reactor.Parent = this;
                    reactor.Root = this.Root;

                    // Add to children list
                    this.Children.Add(reactor);

                    this.Resize += reactor.OnParentResize;
        ...
        public virtual void Remove(RenderReactor reactor)
        {
            if (reactor != null)
            {
                // Remove from children list
                this.Children.Remove(reactor);
        ...
        public virtual bool Contains(RenderReactor reactor, bool recursive)
        {
            if (this.Children != null)
            {
                foreach (var c in this.Children)
                {
                    ...
                    if (recursive && c.Contains(reactor, true))
                    {
                        return true;
                    }
                }
        ...
        public event EventHandler<RenderReactorDrawEventArgs> BeginDrawStarted = delegate { };

        public event EventHandler<RenderReactorDrawEventArgs> EndDrawStarted = delegate { };
        ...
        private void OnBeginDrawStarted(RenderReactorDrawEventArgs e)
        {
            this.BeginDrawStarted?.Invoke(this, e);
        }

        private void OnEndDrawStarted(RenderReactorDrawEventArgs e)
        {
        ...
        public sealed override void BeginDraw(IMMEngineGraphicsService graphics, GameTime time, byte alpha)
        {
            ...
            this.OnBeginDrawStarted(
                new RenderReactorDrawEventArgs(this.RenderTarget, this.RenderTargetDestinationRectangle, time));
