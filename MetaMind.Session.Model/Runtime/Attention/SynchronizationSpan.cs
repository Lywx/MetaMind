namespace MetaMind.Session.Model.Runtime.Attention
{
    ...
    {
        public static SynchronizationSpan Zero => new SynchronizationSpan(DateTime.Now, TimeSpan.FromHours(0), DateTime.Now);
        ...
        public TimeSpan Duration
        {
            get
            {
                if (this.IsEnded)
                {
                    return this.CertainDuration;
                }

                return this.CertainDuration + TimeSpan.FromSeconds((long)((DateTime.Now - this.RecentStartTime).TotalSeconds * this.Accelaration));
            }
        }
