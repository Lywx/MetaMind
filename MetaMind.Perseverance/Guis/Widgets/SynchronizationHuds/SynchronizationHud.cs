...
{
    public class SynchronizationHud : InputObject // refactoring to graphics components
    {
        //---------------------------------------------------------------------
        ...
        {
            FontManager.DrawCenteredText( settings.AccelerationFont, "x", AccelerationPrefixPosition, settings.AccelerationColor, 1f );
            FontManager.DrawCenteredText( settings.AccelerationFont, string.Format( "{0}", synchronization.Acceleration.ToString( "F1" ) ), AccelerationSubfixPosition, settings.AccelerationColor, settings.AccelerationSize );
        }
        ...
        private void DrawMassage( GameTime gameTime )
        {
            ...
            {
                var colorBase = settings.BarFrameAscendColor;

                FontManager.DrawCenteredText(
                settings.MessageFont,
                "Look like you are gonna be more happier from today.",
                MessagePosition,
                colorBase.UpdateByTransitionAlpha( ( byte ) ( 255 * Math.Abs( Math.Sin( ( gameTime.TotalGameTime.TotalSeconds * 3 ) ) ) ) ),
                settings.MessageSize
                );
            }
            else
            {
                var colorBase = settings.BarFrameDescendColor;
